...
I have this newly created Hanami application using the [Template repository](/episodes/1-creating-hanami-application).
...
It may be a lot comparing to simplified, [MVC](https://www.tutorialspoint.com/mvc_framework/mvc_framework_introduction.htm) approach, where There are only three parts of the system - Model, View, and Controller, but history proved that `MVC` just does not scale well and extended Hanami architecture, where each class has it's dedicated purpose definitely allows to reduce coupling in bigger systems and better manage complexity.
